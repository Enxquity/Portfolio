...
function Camera:KnitStart()
    ...
    local CameraShaker = self.Shake.new(Enum.RenderPriority.Camera.Value, function(ShakeCFrame)
        self.Camera.CFrame = self.Camera.CFrame * ShakeCFrame
    end)
    CameraShaker:Start()

    self.Services.MovesManager.Shake:Connect(function(ShakePreset)
        CameraShaker:Shake(self.Shake.Presets[ShakePreset])
    end)
end 
